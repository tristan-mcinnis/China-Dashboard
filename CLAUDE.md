...
- `elite_press.py`: Sinocism-style institutional sources via Google News RSS proxy, each tagged with a thematic `pillar` — party organs/official media (People's Daily, Qiushi → `politics`), domestic financial media (Caixin, Yicai, 21CBH → `economy`), tech/industry media (36Kr, Jiemian → `tech`), Western elite press on China (FT, WSJ, Bloomberg, Reuters, SCMP → `geopolitics`), the English-facing Chinese state media (China Daily, CGTN, Global Times → `geopolitics`, i.e. the official English framing aimed abroad — their own native RSS is dead/frozen, so they ride the same Google News proxy), and an English society desk (Sixth Tone → `society`). Chinese headlines translated with DeepSeek; English passed through.
